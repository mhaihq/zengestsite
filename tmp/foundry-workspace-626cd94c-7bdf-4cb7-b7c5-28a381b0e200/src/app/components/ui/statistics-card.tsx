...
const Stats = () => {
  return (
    <section className="py-20 md:py-32 w-full px-4 md:px-6">
      <style>{css}</style>
      <div className="container mx-auto">
        <div className="mx-auto max-w-2xl text-center mb-16">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-medium tracking-tight text-slate-900 dark:text-white mb-6">
            We don't believe in talk we Deliver Results
          </h2>
          <p className="text-lg text-slate-500 dark:text-slate-400 max-w-xl mx-auto leading-relaxed">
            See how Hana's voice AI outperforms traditional call centers and competitors in efficiency and conversion rates.
          </p>
        </div>
        <div className="relative mx-auto mt-20 flex h-[500px] max-w-4xl items-center justify-center gap-4 md:gap-8">
          {[
            { value: 35, label: "Legacy IVR", delay: 0.2 },
            { value: 48, label: "Call Centers", delay: 0.4 },
            {
              value: 99,
              label: "Hana AI",
              className: "bg-[#A7BCF5]",
            ...
            { value: 62, label: "Other AI", delay: 0.8 },
          ].map((props, index) => (
            <motion.div
              key={index}
    ...
    <div className="group relative h-full w-full flex flex-col justify-end">
      <div className="candy-bg relative h-full w-full overflow-hidden rounded-[40px] bg-slate-100 dark:bg-slate-800/50">
        ...
        >
          <div className="relative flex h-14 w-full items-center justify-center gap-2 rounded-full bg-white/20 tracking-tighter">
            <NumberFlow value={value} suffix="%" className="text-lg font-medium" />
          </div>
        </motion.div>
      </div>
      ...
      >
        <motion.div
          ...
          transition={{ duration: 0.5, type: "spring", damping: 15, delay }}
          className={cn(
            "absolute -top-12 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-xl bg-zinc-800 px-3 py-1.5 text-white font-medium text-sm shadow-lg",
